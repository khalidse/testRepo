package com.univision.radio.zap;
...
public class ZapManager {
	...
	private static final String UPSTREAM_PROXY_HOST = null;
	private static final int UPSTREAM_PROXY_PORT = 0;
	Process process;
	...
	public int startZAP(String zapPath, String proxyHost, int proxyPort) throws Exception {
		if (process == null) {
			...
			params.add("tmp");
			params.add("-config");
			params.add("scanner.threadPerHost=20");
			params.add("-config");
			params.add("spider.thread=10");
			params.add("-config");
			params.add("api.key=" + API_KEY);
			String upstreamProxyHost = UPSTREAM_PROXY_HOST;
			if (upstreamProxyHost != null) {
				int upstreamProxyPort = UPSTREAM_PROXY_PORT;
			...
			waitForSuccessfulConnectionToZap(proxyHost, port);
		} else {
	...
	public void stopZap(String proxyHost, int proxyPort) {
		if (process == null)
			return; // ZAP not running
		try {
			log.info("Stopping ZAP");
			ClientApi client = new ClientApi(proxyHost, proxyPort);
	...
	private void waitForSuccessfulConnectionToZap(String proxyHost, int proxyPort) {
		...
		do {
			try {
				...
				socket = new Socket();
				socket.connect(new InetSocketAddress(proxyHost, port), connectionTimeoutInMs);
				...
				try {
					...
					throw new RuntimeException("The task was interrupted while sleeping between connection polling.",
							e);
				}
				...
				if (ellapsedTime >= timeoutInMs) {
					throw new RuntimeException(
							"Unable to connect to ZAP's proxy after " + timeoutInMs + " milliseconds.");
				}
